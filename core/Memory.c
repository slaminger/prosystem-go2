...
//static uint8_t memory_rom[MEMORY_SIZE] = {0};
...
{
   if(address < 0x4000 /*!memory_rom[address]*/)
   {
      switch(address)
      {
         case WSYNC:
         ...
         default:
            ...
            if(address >= 0x2040 && address <= 0x20ff)
                memory_ram[address - 0x2000] = data;
            else if(address >= 0x2140 && address <= 0x21ff /*8702 ??*/)
                memory_ram[address - 0x2000] = data;
            else if(address >= 0x0040 && address <= 0x00ff)
                memory_ram[address + 0x2000] = data;
            else if(address >= 0x140 && address <= 0x01ff)
                memory_ram[address + 0x2000] = data;
            break;
